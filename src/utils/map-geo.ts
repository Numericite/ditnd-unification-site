// ─── Table département → { nom, région } ─────────────────────────────────────

const DEPARTEMENTS: Record<string, { name: string; region: string }> = {
...
// ─── Extraction du code département depuis le code postal ─────────────────────

export function getDepartementCode(postalCode: string): string {
...
// ─── Accès aux infos d'un département ────────────────────────────────────────
...
export function getDepartementInfo(code: string): DepartementInfo | null {
	const entry = DEPARTEMENTS[code];
	if (!entry) return null;
	return { code, ...entry };
}

export function getMarkerGeo(
	postalCode: string | null | undefined,
): DepartementInfo | null {
	if (!postalCode) return null;
	const code = getDepartementCode(postalCode);
	return getDepartementInfo(code);
}
